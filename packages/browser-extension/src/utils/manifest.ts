...
export async function storeManifestInfo(workspaceUri: string, data: ManifestData) {
...
export async function getManifestInfo(): Promise<StoredManifestInfo | undefined> {
  const info: StoredManifestInfo | undefined = await chromeStorage.get(StoreManifestInfo);
  return info;
}
